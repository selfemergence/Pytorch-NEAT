...
import activation
params['hidden_act_fn'] = activation.tanh
params['output_act_fn'] = activation.tanh
params['NODE_MUTATE_PROB'] = 0.1 # 0.03, 0.8
params['CONN_MUTATE_PROB'] = 0.5

# continuous actions
global_log_std = -0.5 * np.ones((4,), dtype=np.float32)
global_std = np.exp(global_log_std)

def act_continuous(pred):
    low, high = -1.0, 1.0 # -1 and 1 are boundaries of tanh
    action = pred + np.random.uniform(low, high, size=pred.shape) * global_std
    action = np.clip(action, low, high)
    return action



def run_agent(agent, env, epochs=10, steps=200, render_test=False):
    ...
    for epoch in range(epochs):
        ...
        while not done:
            ...
            pred = np.array(output)#*2-1 # for continuous actions
            # act continuous
            action = act_continuous(pred)
            #action = pred
...
def main():
    ...
    # save stats to file
    np.savetxt(directory + env_name + '-fitness-st-' + str(seed)+'.txt', list(zip(best_fitness, 
                                                            avg_fitness)), \
    ...
    plt.savefig(directory + env_name + '-reward-st-' + str(seed) +'.png')
    ...
    with open(directory + env_name + '-bests-st-' + str(seed) +'.plt', 'wb') as f:
        pickle.dump(sim.stats['best'], f)
    
    #Draw best Network
    img = solution.network.draw(1000, 800)
    outpath = directory + env_name + '-network-st-' + str(seed) + '.png'
    ##save the image
