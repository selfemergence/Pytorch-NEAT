...
def run_agent(agent, env, epochs=100, seed=2021, render_test=False):
...
class Simulation:
    ...
    def __init__(self, env_name, pop_size, epochs=10, seed=2021, verbosity=1):
        ...
        self._maxSteps = self.env.spec.max_episode_steps
        self.epochs = epochs
...
#                self.pop.members[i].fitness = fitnesses[i]
            ...
            for i in range(self.pop.size()):
                ...
                agent.fitness = run_agent(agent, self.env)
...
#    
#    # save stats to file
#    np.savetxt(directory + env_name + '-fitness-' + str(seed)+'.txt', list(zip(best_fitness, 
#                                                            avg_fitness)), \
...
#        cv2.destroyAllWindows()
    ...
    import pickle
    # save best weights for future uses
    file = open(directory + env_name + '-bests-' + str(seed) + '.plt','rb')
